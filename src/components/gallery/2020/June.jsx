...
import june2020csv from '../../../assets/data/june2020.csv?url';  
...
const June = () => {
  ...
  useEffect(() => {
    d3.csv(june2020csv).then((result) => {
      setData(result);
  ...
  employmentData.forEach((occupation) => {
    ...
    barGroup.on('mousemove', (event) => {
      tooltip.html(displayToolTip(occupation));
      tooltip.style('display', 'inline')
        .style('left', tooltipLeft(event, document.getElementsByClassName('tooltip')[0]))
        .style('top', tooltipTop(event, document.getElementsByClassName('tooltip')[0]));
    })
