...
import { MapContainer, TileLayer, ZoomControl, Popup, useMapEvent, useMap } from 'react-leaflet';
...
// New component to handle the FeatureLayer
function EsriFeatureLayer({ url, style, onClick }) {
  const map = useMap();
  ...
  useEffect(() => {
    const featureLayer = EsriLeaflet.featureLayer({
      url: url,
      style: (feature) => {
        const color = legend.find((option) => 
            ...
            <Popup position={latLng}>
              ...
              <ul className="tooltip__list">
                <li className="tooltip__text">
                  {selectedZone.dupac_eff ? 
                    `${selectedZone.dupac_eff} effective maximum dwelling units per acre` : 
                    'Effective maximum dwelling units per acre data not available'
                  }
                </li>
              </ul>
            </Popup>
